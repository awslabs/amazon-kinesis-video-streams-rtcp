...
void serialize_senderReport()
{
    ...
    memset(&rtcpPacket, 0x00, sizeof( RtcpPacket_t ));
    memset(&senderReport, 0x00, sizeof( RtcpSenderReport_t ));
    ...
    result = Rtcp_CreatePayloadSenderReport( &ctx, &rtcpPacket, paylaodLength, &senderReport );
    assert( RTCP_RESULT_OK == result );

    result = Rtcp_Serialize( &ctx, &rtcpPacket, pBuffer, &bufferLen );
